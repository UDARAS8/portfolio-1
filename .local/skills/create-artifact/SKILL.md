...
description: "Required first step when creating any new project. Call createArtifact() to scaffold a runnable project (web app, video, slides, mobile)."
---
...
- **Creative / canvas artifacts** — no backend, no OpenAPI, no codegen:
  ...
  - **video-js** (animated videos): Create a video artifact and build following the `video-js` skill. Always delegate the entire build to a DESIGN subagent — do not build the video yourself.
...
**Deployable artifacts** (`react-vite`, `expo`, `data-visualization`, `mockup-sandbox`) — present and suggest deploy:
...
**`slides` and `video-js` artifacts** — present only. Users export these from the preview pane; there is no deploy step. **Never call `suggestDeploy` for these types:**
...
| `mockup-sandbox` | `design` | `web` | `pnpm --filter @workspace/__SLUG__ run dev` | `previewPath` | `static` | `pnpm --filter @workspace/__SLUG__ run build` |  |
